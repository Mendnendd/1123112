        ...
        case 'place_order':
            ...
            try {
                ...
                $tradeData = [
                    ...
                    'notes' => 'Manual Futures Trade'
                ];
                
                $db->insert('trading_history', $tradeData);
                $success = "Futures order placed successfully: {$side} {$quantity} {$symbol}";
                
            } catch (Exception $e) {
                $error = 'Failed to place futures order: ' . $e->getMessage();
            }
            break;
...
// Get trading pairs
$pairs = $db->fetchAll("SELECT * FROM trading_pairs WHERE (trading_type = 'FUTURES' OR trading_type = 'BOTH') AND enabled = 1");
...
try {
    ...
    if (is_array($account)) {
        ...
        $balance = 0;
        error_log("Account info is not an array: " . print_r($account, true));
    }
...
<head>
    ...
    <title>Futures Trading - <?php echo APP_NAME; ?></title>
    ...
    <main class="main-content">
        <div class="page-header">
            ...
            <div class="trading-status">
                ...
                Futures Trading: <?php echo $settings['futures_trading_enabled'] ? 'Enabled' : 'Disabled'; ?>
            </div>
        ...
        <div class="trading-grid">
                ...
                <div class="card-content">
                    <form method="POST" action="">
                        ...
                        <div class="form-actions">
                            <button type="submit" class="btn btn-primary">Place Futures Order</button>
                        </div>
                    </form>
                </div>
            </div>
            ...
            <div class="trading-card">
                <div class="card-header">
                    <h3>📊 Active Futures Positions</h3>
                </div>
                <div class="card-content">
                    <?php if (empty($activePositions)): ?>
                        <div class="empty-state">
                            <div class="empty-icon">📈</div>
                            <p>No active futures positions</p>
                        </div>
                    <?php else: ?>
        ...
        <div class="trading-card full-width">
            <div class="card-header">
                <h3>📊 Recent Futures Trades</h3>
                <a href="trades.php?trading_type=FUTURES" class="btn btn-sm">View All</a>
            </div>
            <div class="card-content">
                <?php
                ...
                ");
                ?>
                <?php if (empty($recentFuturesTrades)): ?>
                    <div class="empty-state">
                        <div class="empty-icon">📊</div>
                        <p>No futures trades yet</p>
                    </div>
                <?php else: ?>
                    <div class="trades-table">
                        <table>
                            ...
                            <tbody>
                                <?php foreach ($recentFuturesTrades as $trade): ?>
                                    <tr>
